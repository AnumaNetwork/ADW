...
*/
    ...
    async native_wallet(...args) {
        ...
        let walletBin = this.manager.getDeployableBinary('anumawallet');
        ...
        switch(cmd) {
            ...
            //     return this.spawn(walletBin,['send',`--kasparov-address=${kasparov}`,`--private-key=${private_key}`,`--to-address=${to_address}`,`--send-amount=${amount}`]);
            // } break;
            // case 'create': {
...
help(term) {
    switch(term) {
        default: {
            ...
            //migrate - upgrades Kasparov database schema\r
            this.terminal.write(`
native-wallet <command> - run native Anuma CLI wallet command, where command is:\r
\r
...
\r
rpc [command] - run RPC command against the default Anumad instance\r
\r
    ...
    build(...args) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            this.spawn(process.argv[0], [path.join(this.appFolder,'build','build.js'), ...args], {
                ...
                let target = this.manager.getDeployableBinary('anumad');
