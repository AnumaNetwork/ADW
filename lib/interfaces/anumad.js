	...
	start() {
		...
		return new Promise(async (resolve,reject) => {
			...
			if (!("uacomment" in args)) {
				...
				this.writeToSink(`ANUMAD: Adding uacomment=${uacomment} to arguments\n`);
			...
			if (this.task.conf['upnpEnabled'] && args['listen'] && args['listen'].indexOf(":") !== -1) {
				...
				if (externalip) {
					if (!("externalip" in args)) {
						this.writeToSink(`ANUMAD: Adding externalip=${externalip} to arguments\n`);
					...
					if (publicPort) {
						...
						this.writeToSink(`ANUMAD: Changing externalip from ${args['externalip']} to ${externalip}:${publicPort} in arguments\n`);
						args['externalip'] = `${externalip}:${publicPort}`;
						if (publicPort !== nodePort) {
							this.writeToSink(`ANUMAD: Changing listener port from ${nodePort} to ${publicPort} in arguments\n`);
							args['listen'] = `${args['listen'].split(':')[0]}:${publicPort}`;
						}
						if (!("maxinpeers" in args)) {
							this.writeToSink(`ANUMAD: Adding maxinpeers=${this.upnpMaxInPeers} to arguments\n`);
			...
			const configFile = path.join(this.folder, 'anumad.conf');
			...
			await this.VSARGS([this.getDeployableBinary('anumad'),'--version']);

			this.run({
				args : async () => {
					let _args_ = [
 						this.getDeployableBinary('anumad')
					];

					if(!this.task.conf['skip-utxoindex']){
						_args_.push('--utxoindex')
					}
					_args_.push('--allow-submit-block-when-not-synced');
					_args_.push('--addpeer=78.141.214.164:12413');
					_args_.push('--addpeer=107.191.50.121:12413');
			...
			resolve();
			// mutes all anumad processes by default
			this.mute = true;
		})
	}
		...
		return new Promise(async (resolve, reject) => {
	        ...
	        try {
				...
				this.data = this.sourcesToData(sources);
	             ...
	             console.log('ANUMAD monitor:'.bold,ex);
	            return reject(ex);
	        }
