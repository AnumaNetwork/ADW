...
class KDXApp extends FlowApp{
	render(){
		let walletMeta = {"generator":"ADW"}
		let list = [
			['Anuma','MIT','Copyright (c) 2024 Anuma Developers'],
...
//			['Mosquitto','EDL-V10 EPL-V10','Copyright (c) 2007, Eclipse Foundation, Inc. and its licensors'],
		...
		let donationAddresses = [
			...
			//["Mining address:", "anuma:qpwlucat86aj6ydcfnha8f5x0dwke2qs3609vm7huy9jg8h7jc72uvz6t24qr"],
			["WebWallet donations:", "anuma:qpwlucat86aj6ydcfnha8f5x0dwke2qs3609vm7huy9jg8h7jc72uvz6t24qr"],
		]
		return html`
		<flow-caption-bar
			logo="/resources/images/anuma-logo-light-bg.png" style="padding:5px 0;">ANUMA WALLET</flow-caption-bar>
		<tab-content for="home">
			...
			<div id='kaspa-resources'>
				...
				<flow-expandable>
					<div slot="title" is="i18n-div" caption>ANUMA SOCIAL & RESOURCES</div>
					<ul style="font-size: 12px;">
						<li>
							<flow-shell-link href="https://t.me/+I_nxFsUGsAk5ZDA0">
								<flow-i18n>Telegram</flow-i18n>
							</flow-shell-link>
						</li>
						<li><flow-shell-link
							href="https://github.com/AnumaNetwork"><flow-i18n>GitHub</flow-i18n></flow-shell-link></li>
						<li><flow-shell-link
							href="https://discord.gg/8ZT93HctbQ"><flow-i18n>Discord Chat</flow-i18n></flow-shell-link></li>
						<li><flow-shell-link
							href="https://x.com/anuma_official"><flow-i18n>Twitter</flow-i18n></flow-shell-link></li>
							<li><flow-shell-link
								href="https://anuma.network"><flow-i18n>Web</flow-i18n></flow-shell-link></li>					
						<li><flow-shell-link
							href="https://explorer.anuma.network"><flow-i18n>Explorer</flow-i18n></flow-shell-link></li>
						<li><flow-link
			...
			<flow-expandable class="donation-addresses" no-info _icon="fal:donate">
				...
				<p is="i18n-p">
					if you wish to further the development of the anuma ecosystem, we accept anuma donations at the following addresses:
				</p>
				${
			...
			<flow-form-control icon="fal:copyright">
				<flow-i18n>Anuma Wallet &amp; Anuma Copyright (c) 2024 Anuma Developers<br/>
		...
		<tab-content for="settings">
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					Data Folder location is used for storage by all Anuma Wallet modules. 
					In default configuration this includes Anumad blockchain data and Kasparov API database.
			...
			<flow-form-control icon="fal:tools">
				...
				<p slot="info" is="i18n-p">
					Advanced mode allows you to manually configure, interact with and monitor Anuma Wallet services.
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:cog" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">
					When enabled, Anuma Wallet runs itself and it's services hidden in the background
			...
			<flow-form-control icon="fal:cog" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">When Autocompound enabled, Anuma Wallet compound UTXOs when there will be compoundable count of unspent transaction outputs (UTXOs).'Use latest Change address' will compund UTXOs using current/latest change address instead of first change address. </p>
			</flow-form-control>
			<flow-form-control icon="fal:cube" class="advanced-tool"
				...
				<p slot="info" is="i18n-p">
					The Enable Mining option starts / stops all configured Anumaminer instances.
				</p>
			</flow-form-control>
			<!-- flow-form-control icon="fal:drafting-compass" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">
					You can stream Anuma Wallet metrics to your own StatsD-compatible server.
				</p>
			</flow-form-control-->
			...
			<flow-form-control id="settings-script" icon="fal:tasks">
				...
				<p slot="info" is="i18n-p">
					Configuration editor allows you to customize Anuma Wallet environment.
					Anuma Wallet configuration is represented in JSON
			...
			<flow-form-control id="settings-templates" icon="fal:file-alt"
				...
				<p slot="info" is="i18n-p">
					Configuration templates allow you to load pre-made Anuma Wallet configurations.
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					It will delete datadir (Data Folder) and restart anumad node to re-sync
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					Start the anumad node without enabling the '--utxoindex' parameter until the synchronization is complete, re-enable this parameter. (Can be used to fix inaccurate balance display)
				</p>
			</flow-form-control>
...
//		this.checkForUpdates().then();
	...
	initRPC(){
		let rpc = new FlowRPC({bcastChannel:'adw'});
		this.rpc = rpc;
	...
	async initManager(){
		...
		manager.on("task-start", (daemon)=>{
			...
			this.refreshApps();
			const {wallet} = this;
			if(!wallet || daemon.task.type!='anumad' || !this.rpcDisconnect){
				return
			}
		...
		manager.on("before-interrupt", ({daemon, interrupt})=>{
			...
			const {wallet} = this;
			if(!wallet || daemon.task.type!='anumad')
				return
	...
	async initWallet() {
		...
		let settings = await this.get_default_local_anumad_settings();
	...
	async get_default_local_anumad_settings() {
		
		let {config:daemons} = await this.get("get-modules-config");
		console.log("############### DAEMONS", daemons);
		let anumad = Object.entries(daemons).map(([k,v]) => { 
			const { args } = v;
	...
	initReleaseNotes(){
		...
		dialog.content = 
`#	Welcome to Anuma Wallet ${pkg.version}!

Useful resources:
- Anuma Discord: https://discord.gg/vMT39xB
- Anuma Telegram: https://t.me/+I_nxFsUGsAk5ZDA0
- Anuma Twitter (X): https://x.com/anuma_official
- Anuma GitHub: https://github.com/AnumaNetwork
- Anuma Wallet GitHub: https://github.com/AnumaNetwork/adw
...
${changelogContent}`;
	...
	async initDaemons(daemons){
		...
		let {params} = this.getModuleArgs('anumad:', daemons);
