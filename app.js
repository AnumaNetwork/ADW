...
class App extends FlowApp{
	...
	async initConfig(){
		...
		if(reset) {
			try {
				...
				alert('Error resetting Anuma Desktop Wallet data folder: '+ex);
			}
	...
	*/
	dataDirInitError(){
		console.log(`Please start app with --init=/path/to/data/dir or --init for default (~/.adw/data)`);
		this.exit();
	}
	...
	*/
	...
	async removeDataDir(){
		try {
			let datadir2path = path.join(this.dataFolder, "anumad-aw0", "anuma-mainnet", "datadir2");
	...
	*/
	...
	getDefaultConfig(){
		...
		Object.entries(config.modules).forEach(([k,v]) => {
			...
			if(['anumad','kasparovd','kasparovsyncd','anumaminer'].includes(type)) {
	...
	setSkipUTXOIndex(skipUTXOIndex){
		...
		return this.setModuleArgs("anumad:", {}, {"skip-utxoindex":skipUTXOIndex})
	}
	getSkipUTXOIndex(){
		let {args, params} = this.getModuleArgs("anumad:");
	...
	setUpnpIfMissing(upnpEnabled) {
		if (this.config?.modules) {
			Object.keys(this.config.modules).forEach((k) =>{
				if (k.startsWith('anumad:') && !('upnpEnabled' in this.config.modules[k])) {
